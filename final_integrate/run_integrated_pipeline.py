#!/usr/bin/env python3
"""
End-to-end PI-VLA integration (Isaac Gym + SAM + obj-loc + TracIK + NTField).

Run from PI-VLA root:
  python final_integrate/run_integrated_pipeline.py \\
    --ntfield_checkpoint ntrl-demo/Experiments/.../Model_Epoch_*.pt \\
    --sam_checkpoint img2objloc_model/checkpoints/sam_vit_h_4b8939.pth \\
    --objloc_checkpoint img2objloc_model/checkpoints_xy/best_xy_model.pt
...
def _infer_objloc_on_crop(
    ...
    import train_objloc_xy as train_mod  # noqa: WPS433
    ...
    model = train_mod.build_model(backbone, pretrained=False).to(dev)
    ...
    with torch.no_grad():
        pred = model(x).float().cpu().numpy().reshape(2)
...
def main() -> None:
    ...
    parser.add_argument("--output_dir", type=str, default=None, help="Session dir (default: output/final_integrate/TIMESTAMP)")
    parser.add_argument("--object_z", type=float, default=0.18, help="Actor Z (m), same order as benchmark")
    ...
    sam_work = os.path.join(session_dir, "sam_work")
    os.makedirs(sam_work, exist_ok=True)
    ...
    # --- SAM + metadata (world XY for training alignment) ---
    sys.path.insert(0, _IMG2OBJ)
    from extract_table_with_sam import (  # noqa: WPS433
        build_mask_generator,
        process_one_image_array,
    )
    ...
    sam_a = _SamArgs()
    sam_a.points_per_side = 32
    sam_a.pred_iou_thresh = 0.88
    sam_a.stability_score_thresh = 0.92
    sam_a.min_mask_region_area = 1000
    sam_a.crop_margin_ratio = 0.06
    sam_a.label_z = 0.1
    sam_a.device = args.sam_device
    sam_a.model_type = "vit_h"
    sam_a.sam_checkpoint = _resolve_under_root(args.sam_checkpoint)

    mask_gen = build_mask_generator(
        argparse.Namespace(
            device=sam_a.device,
            model_type=sam_a.model_type,
            sam_checkpoint=sam_a.sam_checkpoint,
            points_per_side=sam_a.points_per_side,
            pred_iou_thresh=sam_a.pred_iou_thresh,
            stability_score_thresh=sam_a.stability_score_thresh,
            min_mask_region_area=sam_a.min_mask_region_area,
            crop_margin_ratio=sam_a.crop_margin_ratio,
        )
    )
    process_one_image_array(
        rgb_top,
        ...
        sam_a,
        mask_gen,
        object_location_xy=np.array([ox, oy], dtype=np.float64),
    )
    sam_meta = os.path.join(sam_work, "object_location.json")
    if os.path.isfile(sam_meta):
        shutil.copy2(sam_meta, os.path.join(session_dir, "object_location_sam_sidecar.json"))

    for src_name, dst_name in [
        ("first_image_mask.png", "sam_mask.png"),
        ("first_image_overlay.png", "sam_overlay.png"),
        ("first_image_table_crop.png", "sam_table_crop.png"),
    ]:
        s = os.path.join(sam_work, src_name)
        d = os.path.join(session_dir, dst_name)
        if os.path.isfile(s):
            shutil.copy2(s, d)

    crop_for_model = os.path.join(session_dir, "sam_table_crop.png")
    ckpt_obj = torch.load(_resolve_under_root(args.objloc_checkpoint), map_location="cpu")
    z_fixed = float(ckpt_obj.get("z_fixed", 0.1))

    pred_x, pred_y = _infer_objloc_on_crop(
        crop_for_model,
    ...
    object_location_predicted = [float(pred_x), float(pred_y), z_fixed]
    ...
    with open(os.path.join(session_dir, "object_location_predicted.json"), "w", encoding="utf-8") as f:
        json.dump({"xyz_m": object_location_predicted, "note": "z uses z_fixed from objloc checkpoint"}, f, indent=2)
    ...
    with open(os.path.join(session_dir, "q_goal_predicted.json"), "w", encoding="utf-8") as f:
        json.dump(
            {"joint_rad": None if q_goal_pred is None else q_goal_pred.tolist()},
            f,
            indent=2,
        )
