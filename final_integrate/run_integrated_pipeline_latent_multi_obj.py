...
Run from PI-VLA root::
  python final_integrate/run_integrated_pipeline_latent_multi_obj.py \
    ...
    --latent_checkpoint /home/hojinsohn/VLM-NT/PI-VLA/final_integrate/_training_live_latent_cvae_bs256_lr1em4_ep50_20260504_132725.pth

If ``--output_dir`` is set, outputs go under ``<output_dir>/<timestamp>/``.
Otherwise under ``output/final_integrate/<timestamp>/``.

With ``--num_trials`` > 1, each run is written under ``<session>/trial_NN/`` and
aggregate metrics are saved as ``batch_summary.json``, ``batch_trials.jsonl``,
and ``batch.log`` in the session directory.
...
# Table size and sim timestep (aligned with hanwen_grasping collect scripts;
# see trajectory_evaluation.comparison.run_rrt_ntfield_benchmark).
TABLE_DIMS_X = 0.8
TABLE_DIMS_Y = 1.0
TABLE_DIMS_Z = 0.10
_SIM_DT = 1.0 / 60.0
...
def _write_batch_summary(
    ...
    """Write ``batch_summary.json`` and return the aggregate dict."""
    n = len(trial_rows)
...
def _get_latent_model(output_dim: int, vocab_size: int, text_embed_dim: int = 32):
    ...
    class _StudentModelWonorm(nn.Module):
        def __init__(self):
            ...
            # Matches full_train_multi.py (StudentHeadWonorm -> head.net.* keys).
...
def _infer_latent_on_image(
    ...
    Uses the same CVAEStudent architecture as training and enforces strict
...
def main() -> None:
    ...
    def _ee_to_primary_object_distance_m(
        ...
        """
        Euclidean distance (m) between the UR EE link and the root rigid body of
        ``object_handles[0]`` after a short settle. Returns (dist, ee_link_name, target_body_label).
        """
        ...
        try:
            ...
            for cand in ("wrist_3_link", "tool0", "ee_link", "robotiq_arg2f_base_link"):
                if cand in rb_ur:
                    ...
                    break
            if ee_name is None:
                keys = sorted(rb_ur.keys())
                return None, None, f"no_known_ee_link among {keys[:20]}"
            ...
            body_label = inv.get(root_idx, str(root_idx))
            T_obj = gymapi.Transform.from_buffer(st_o["pose"][root_idx])
    ...
    parser.add_argument(
        ...
        default=1,
        help=(
            "How many independent scenes to simulate. When >1, outputs go under "
            "<session>/trial_NN/; batch_summary.json, batch_trials.jsonl, and batch.log "
            "are written under the session directory. With --seed, layout RNG is "
            "reset per trial as seed + trial_index."
        ),
    )
    parser.add_argument(
        "--stop_on_error",
        action="store_true",
        help="Abort the batch on the first trial exception (default: log and continue).",
    )
    parser.add_argument(
        "--ee_settle_steps",
        type=int,
        default=10,
        help="PhysX steps after NTField video playback before measuring EE vs object (default: 10).",
    )
    parser.add_argument(
        "--ee_success_thresh_m",
        type=float,
        default=None,
        help="If set (>0), write success_check.success = (ee_to_target_dist_m < thresh). "
        "Does not change summary['status'] (still latent_tol based).",
    )
    ...
    for trial_idx in range(num_trials):
        trial_dir = session_dir if num_trials == 1 else os.path.join(session_dir, f"trial_{trial_idx:02d}")
        if num_trials > 1:
        ...
        try:
            ...
            # ── Warm-up simulation (settle arm + objects before capture) ────────────
            for t in range(2000):
            ...
            top_view_path = os.path.join(trial_dir, "top_view.png")
            ...
            with open(os.path.join(trial_dir, "object_location.json"), "w", encoding="utf-8") as f:
                json.dump(
                    {
            ...
            ee_m, ee_link, tgt_body = _ee_to_primary_object_distance_m(
                ...
                wj2,
                wj3,
                int(args.ee_settle_steps),
            )
            summary["ee_to_target_dist_m"] = ee_m
            thresh = args.ee_success_thresh_m
            ee_ok = (
                isinstance(thresh, (int, float))
                and float(thresh) > 0.0
                and ee_m is not None
                and float(ee_m) < float(thresh)
            )
            summary["success_check"] = {
                ...
                "success": bool(ee_ok) if isinstance(thresh, (int, float)) and float(thresh) > 0.0 else None,
            }

            # ── Save outputs ─────────────────────────────────────────────────────────
            with open(os.path.join(trial_dir, "pipeline_summary.json"), "w", encoding="utf-8") as f:
                json.dump(summary, f, indent=2)

            trial_rows.append(dict(summary))
            with open(batch_trials_jsonl, "a", encoding="utf-8") as jf:
