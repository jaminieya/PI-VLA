#!/usr/bin/env python3
"""
Multi-object test harness for latent-goal NTField planning with text prompting.
"""
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="PI-VLA multi-object latent planner with text prompt")
    ...
    parser.add_argument("--text_prompt", type=str, default="", help="E.g. 'grasp banana'")
    parser.add_argument("--target_object_idx", type=int, default=0, help="Fallback if text_prompt is empty")
    ...
    dev_nt = torch.device("cpu" if args.ntfield_device == "cpu" or not torch.cuda.is_available() else args.ntfield_device)
    ...
    gym_args = gymutil.parse_arguments(description="latent_prompt", headless=True, custom_parameters=[])
    ...
    # Determine target index from text prompt
    target_idx = args.target_object_idx
    if args.text_prompt:
        found_idx = -1
        for i, name in enumerate(object_slot_names):
            if name.lower() in args.text_prompt.lower() or args.text_prompt.lower() in name.lower():
                found_idx = i
                break
        if found_idx != -1:
            target_idx = found_idx
            print(f"Matched prompt '{args.text_prompt}' to object '{object_slot_names[target_idx]}' (index {target_idx})")
        else:
            print(f"Warning: Could not match prompt '{args.text_prompt}' to any object. Defaulting to index {target_idx}")
    
    print(f"Target: [{target_idx}] {object_slot_names[target_idx]}")
    ...
    # Multi-Object Student Model Inference
    dev_latent = torch.device("cuda" if torch.cuda.is_available() and args.latent_device == "auto" else args.latent_device)
    student_model = StudentModelWonormMulti(output_dim=256, num_objects=3).to(dev_latent)
    ckpt = torch.load(_resolve_under_root(args.latent_checkpoint), map_location=dev_latent)
    student_model.load_state_dict(ckpt["model_state_dict"])
    student_model.eval()

    img_tensor = _process_img(rgb_top, 224).unsqueeze(0)
    img_tensor = transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])(img_tensor).to(dev_latent)

    with torch.no_grad():
        latents_pred = student_model(img_tensor)  # (1, 3, 256)
        latents_pred = latents_pred.squeeze(0).cpu().numpy()  # (3, 256)

    # Select the latent goal corresponding to the text prompt's target index
    latent_goal_pred = latents_pred[target_idx]
    ...
    def _annotate_ntfield_frames(frames_list: List[np.ndarray], playback_label: str) -> None:
        ...
        prompt_str = args.text_prompt if args.text_prompt else f"Target idx: {args.target_object_idx}"
        line_blocks = [
            textwrap.wrap(f"Prompt: {prompt_str}", width=wrap_w) or [f"Prompt: {prompt_str}"],
        ...
        for fi, fr in enumerate(frames_list):
            ...
            for line in lines:
                for dx, dy in ((-2, 0), (2, 0), (0, -2), (0, 2)):
                    cv2.putText(bgr, line, (10 + dx, y + dy), font, scale, (0, 0, 0), thick + 2, cv2.LINE_AA)
                cv2.putText(bgr, line, (10, y), font, scale, (255, 255, 255), thick, cv2.LINE_AA)
                y += line_gap
