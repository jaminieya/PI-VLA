...
if __name__ == '__main__':
    ...
    for episode_idx in range(num_episodes):
        ...
        obstacle_objs = []
        GT_OBJ_POS_LIST = []
        # Target pos constraint to keep objects away from the direct grasping zone 
        # (matching the original initialization logic)
        GT_TARGET_POS = [np.random.uniform(max(table_x_min, 0.20 + table_dims.x/2), table_x_max),
                         np.random.uniform(table_y_min, table_y_max),
                         table_dims.z + 0.08]
        ...
        for k in range(NUM_OF_OBJECTS):
            ...
            while is_collision:
                ...
                for placed_obj in obstacle_objs:
                    rdata = fcl.CollisionData(request=req)
                    fcl.collide(temp_obj, placed_obj, req, rdata)
                    if rdata.result.is_collision:
                        is_collision = True
                        break
                        
                if not is_collision:
                    # distance to target constraint
                    dist = np.sqrt((tx - GT_TARGET_POS[0])**2 + (ty - GT_TARGET_POS[1])**2)
                    if dist <= 0.2:
                        is_collision = True
                        continue
            ...
            # Move physical actor in Isaac Gym
            handle = object_handles[k]
            states = gym.get_actor_rigid_body_states(envs[-1], handle, gymapi.STATE_POS)
            states['pose']['p']['x'] = tx
            states['pose']['p']['y'] = ty
            states['pose']['p']['z'] = tz
            # Zero-out rotation to match original static spawn so pre-computed grasps stay valid
            states['pose']['r']['x'] = 0.0
            states['pose']['r']['y'] = 0.0
            states['pose']['r']['z'] = 0.0
            states['pose']['r']['w'] = 1.0
        ...
        for settle_step in range(100):
            ...
            # On the final step, extract exact settled poses to build the collision meshes for RRT
            if settle_step == 99:
                for i_obj, element in enumerate(object_handles):
                    ...
                    # update object tracking centers for this episode
        ...
        if init2grasp_path is None:
            print("No valid grasp found in this episode. Continuing.")
            continue
        ...
        start_joint_config = None
        start_image = None
        goal_joint_config = np.array(init2grasp_path[-1][:JOINT_DIM], dtype=np.float32)
        ...
        start_joint_config = np.array(dof_states['pos'][:JOINT_DIM], dtype=np.float32)
        ...
        if start_image is not None and start_joint_config is not None:
            ...
            with h5py.File(out_path, "w") as f:
                ...
                f.create_dataset("start_joint_config", data=start_joint_config)
                f.create_dataset("object_location", data=object_location)
                f.create_dataset("goal_joint_config", data=goal_joint_config)
