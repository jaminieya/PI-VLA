...
#!/bin/bash
# Run collect_multi_obj_layout_only.py jobs in parallel on 3 GPUs.
# Usage: ./run_3_gpu_parallel.sh [episodes_per_worker]
#   episodes_per_worker: episodes per worker (default: 334); total = 3 x instances_per_gpu x episodes_per_worker
#
# Examples:
#   ./run_3_gpu_parallel.sh           # 334 episodes per worker (= 2004 total)
#   ./run_3_gpu_parallel.sh 100       # 100 episodes per worker (= 600 total)

set -e
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$SCRIPT_DIR"

EPISODES_PER_WORKER="${1:-334}"
NUM_GPUS=3
INSTANCES_PER_GPU=2

echo "Multi-obj layout only | $NUM_GPUS GPUs x $INSTANCES_PER_GPU instances, $EPISODES_PER_WORKER episodes each (= $((NUM_GPUS * INSTANCES_PER_GPU * EPISODES_PER_WORKER)) total)"
echo "Logs: collect_gpu<gpu>_worker<idx>.log"
echo ""
...
for gpu in 0 1 2; do
  for worker in 0 1; do
    CUDA_VISIBLE_DEVICES=$gpu nohup python collect_multi_obj_layout_only.py \
      --headless \
      --num_episodes "$EPISODES_PER_WORKER" \
      --env_id "$WORKER_ID" \
      > "collect_gpu${gpu}_worker${worker}.log" 2>&1 &

    echo "Started GPU $gpu worker $worker (env_id=$WORKER_ID) PID=$!"
