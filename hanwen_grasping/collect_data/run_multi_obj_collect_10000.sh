#!/bin/bash
# Run collect_multi_obj_data_for_student.py 10,000 times across 3 GPUs (parallel).
# Each GPU writes to its own output subtree to avoid timestamp/path collisions.
#
# Usage:
#   cd PI-VLA/hanwen_grasping/collect_data
#   bash run_multi_obj_collect_10000.sh
#   bash run_multi_obj_collect_10000.sh <base_output_dir> <total_runs>
#
# Example:
...
for gpu in 0 1 2; do
  ...
  LOG_FILE="${SCRIPT_DIR}/logs/multi_obj_collect_gpu${gpu}.log"
  ...
  (
    ...
    for i in $(seq 1 "$RUNS_FOR_GPU"); do
      ...
      if ! CUDA_VISIBLE_DEVICES="$gpu" python collect_data/collect_multi_obj_data_for_student.py \
        --headless \
        --output_dir "$WORKER_OUTPUT_DIR"; then
        FAILURES=$((FAILURES + 1))
...
for idx in 0 1 2; do
  echo "  GPU $idx log: $SCRIPT_DIR/logs/multi_obj_collect_gpu${idx}.log"
done
echo "Use: tail -f $SCRIPT_DIR/logs/multi_obj_collect_gpu0.log"
echo ""
