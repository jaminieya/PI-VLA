...
class global_scene:
    ...
    def register_points(self, point_arr):
        for point, weights in point_arr:
            x, y, z = point
            x -= self.offset_[0]
            y -= self.offset_[1]
            z -= self.offset_[2]
            x_index = round(x/0.01)
            y_index = round(y/0.01)
            z_index = round(z/0.01)
            if 0 <= x_index < self.dim_x_ and 0 <= y_index < self.dim_y_ and 0 <= z_index < self.dim_z_:
                if self.scene_[x_index][y_index][z_index] != -1:
                    self.scene_[x_index][y_index][z_index] = 1
    ...
    def is_visible(self, qx, qy, qz):
        x_ind = round((qx - self.offset_[0])/0.01)
        y_ind = round((qy - self.offset_[1])/0.01)
        z_ind = round((qz - self.offset_[2])/0.01)
        return self.scene_[x_ind][y_ind][z_ind] > 0
