#
# File:          new_setup_dataset_collect.py
# Brief:         Same as new_setup.py but captures (image, 6DOF joint) during trajectory
#                and saves to collected_data/grasp_6dof_demo_YYYYMMDD_HHMMSS.h5
#
# Run: cd hanwen_grasping && python new_setup_dataset_collect.py
#      (uses target_idx=0 by default; --target_idx N to grasp object N)
#
...
if __name__ == '__main__':
    ...
    args = gymutil.parse_arguments(
        ...
        custom_parameters=[
            ...
            {"name": "--object_idx", "type": int, "default": None, "help": "Force single object by index (e.g. 5=banana). Overrides TARGET_OBJ_INDEX."},
        ],
    )
    ...
    for i in range(num_of_envs):
        ...
        # Arm DOF handles only (no gripper) for joint recording
        dof_handles_arm = [spj, slj, ej, wj1, wj2, wj3]
        ...
        obj_index_list = [args.object_idx] if getattr(args, "object_idx", None) is not None else TARGET_OBJ_INDEX
        target_file_idx = np.random.choice(obj_index_list, NUM_OF_OBJECTS)
        object_handles = []
    ...
    # Interpolate path for denser capture along trajectory (every waypoint from start to goal)
    INTERPOLATION_STEPS = 20  # intermediate points between each planner waypoint
    if init2grasp_path and len(init2grasp_path) >= 2:
        orig_len = len(init2grasp_path)
        init2grasp_path = interpolate_path(init2grasp_path, steps_between=INTERPOLATION_STEPS)
        print(f"Interpolated to {len(init2grasp_path)} waypoints (from {orig_len} planner waypoints)")

    path_id = 0
    frames_at_waypoint = 0
    SETTLE_STEPS = 30  # steps to let robot settle at each waypoint before capturing
    JOINT_DIM = 6  # arm joints only (no gripper)
    dataset_samples = []
    ...
    if viewer is not None:
        print(f"Will capture {num_waypoints} samples. Keep viewer open, then close to save.")
    else:
        print(f"Will capture {num_waypoints} samples in headless mode.")
    ...
    while viewer is None or not gym.query_viewer_has_closed(viewer):
        ...
        # Capture ONE image per joint_config (waypoint), not entire trajectory.
        # Wait for robot to settle at each waypoint before capturing.
        if path_id < len(init2grasp_path):
            frames_at_waypoint += 1
            if frames_at_waypoint >= SETTLE_STEPS:
                gym.render_all_camera_sensors(sim)
                # Front view (global camera)
                ...
                # Record 6 arm joint positions from simulator (no gripper)
                joint_config = np.array([gym.get_dof_position(envs[-1], h) for h in dof_handles_arm], dtype=np.float32)
                dataset_samples.append({"image": rgb_front, "image_top": rgb_top, "joint_config": joint_config})
                path_id += 1
                frames_at_waypoint = 0
        else:
            # All waypoints captured. Break to save (enables headless exit).
            print("All waypoints captured. Saving data...")
            break
    ...
    if dataset_samples:
        ...
        images_top_arr = np.array([s["image_top"] for s in dataset_samples], dtype=np.uint8)
        joint_configs_arr = np.array([s["joint_config"] for s in dataset_samples], dtype=np.float32)
        num_samples = len(dataset_samples)
        final_joint_config = dataset_samples[-1]["joint_config"].copy()
        object_name = get_object_display_name(object_asset_files[target_file_idx[target_idx]])
        obj_xy = GT_OBJ_POS_LIST[target_idx]
        object_location = np.array([obj_xy[0], obj_xy[1], table_dims.z + 0.08], dtype=np.float32)

        with h5py.File(out_path, "w") as f:
            f.create_dataset("images", data=images_arr, compression="gzip")
            f.create_dataset("images_top", data=images_top_arr, compression="gzip")
            ...
            f.attrs["joint_names"] = "shoulder_pan,shoulder_lift,elbow,wrist_1,wrist_2,wrist_3"
        print(f"Saved {num_samples} samples (front + top images + 6DOF joint_configs + object_name + object_location) to {out_path}")
    else:
