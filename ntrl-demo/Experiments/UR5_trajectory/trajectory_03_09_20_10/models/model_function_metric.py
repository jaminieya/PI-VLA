...
class Function():
    ...
    def LossTrajectory(self, points, tau_obs, beta, epoch, weight_traj=1.0, weight_eikonal=1e-2,
        ...
        """
        Trajectory-only loss for NTField training from RRT paths.
        - Trajectory supervision: (tau_pred - tau_obs)^2
        - Eikonal (F=1): (|nabla T|^2 - 1)^2 for consistent gradient-based planning
        - Optional collision: when collision_callback is provided, adds normal-alignment
          loss so NTField gradient repels from obstacles near collision boundaries.
        """
        tau, w, Xp = self.network.out(points)
        dtau = self.gradient(tau, Xp)
        ...
        # Eikonal (F=1): |nabla T|^2 = 1
        eikonal_loss = weight_eikonal * (torch.mean((S0 - 1) ** 2) + torch.mean((S1 - 1) ** 2))

        loss_n = trajectory_loss + eikonal_loss

        # Optional collision loss: align gradient with obstacle-repulsion when near obstacles
    ...
    def plot(self,epoch,total_train_loss,alpha,source):
        ...
        quad1 = ax.pcolormesh(X,Y,S,vmin=0,vmax=1,shading='auto')
