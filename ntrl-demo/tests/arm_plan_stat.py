...
def Viz_line_pc(centers, radii, colors, env_path, save_path=None):
    import trimesh
    meshpath = env_path
    ...
    if save_path:
        # Headless: save to HTML for viewing in browser
        ...
        print(f'Saved visualization to {save_path} (open in browser)')
    else:
        try:
            scene.show()
        except Exception as e:
            # Fallback when no display/OpenGL (headless, SSH, etc.)
            fallback_path = 'arm_path.html'
            from trimesh.viewer import scene_to_html
            html = scene_to_html(scene)
            with open(fallback_path, 'w') as f:
                f.write(html)
            print(f'Display unavailable ({e}). Saved to {fallback_path} (open in browser)')
