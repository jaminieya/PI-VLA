...
WANDB_RUN_NAME = "wonorm_mse_cos"                          # None → wandb auto-generates
...
INTEGRATION_SCRIPT = PI_VLA_ROOT / "final_integrate" / "run_integrated_pipeline_latent_only.py"
NTFIELD_CHECKPOINT = (
...
RUN_INTEGRATION_EVERY = 5          # run integration eval every N epochs
...
def _save_live_checkpoint_for_integration(model: nn.Module, z_dim: int, ckpt_path: Path) -> None:
    """
...
def _parse_integration_trial_result(session_dir: Path) -> dict:
    ...
    Primary success source is `summary["success_check"]["success"]` written by
    the integration script. We also pass through `ee_to_target_dist_m` so
    training can aggregate average distance-to-target across trials.
    """
    ...
    video = summary.get("videos", {}).get("predicted_latent_goal")
    ...
    ee_to_target_dist_m = success_check.get("ee_to_target_dist_m")

    try:
        ee_to_target_dist_m = (
            float(ee_to_target_dist_m)
            if ee_to_target_dist_m is not None
            else None
        )
    except (TypeError, ValueError):
        ee_to_target_dist_m = None

    # Backward compatibility: if `success_check` doesn't exist, fall back to
    # prior planner/video heuristic so old summaries still parse.
    ...
    return {
        ...
        "ee_to_target_dist_m": ee_to_target_dist_m,
    }
...
def run_integration_eval(
    ...
    _save_live_checkpoint_for_integration(model, z_dim, live_ckpt_path)
    ...
    for trial in range(num_trials):
        ...
        ee_to_target_dist_m = parsed.get("ee_to_target_dist_m")
        if success:
            n_success += 1
        per_trial_times.append(elapsed)
        if ee_to_target_dist_m is not None:
            per_trial_dists.append(float(ee_to_target_dist_m))
        ...
        print(
            ...
            f"success={success} | dist={ee_to_target_dist_m}m | "
        ...
        if _wandb_active():
            ...
            wandb_table_rows.append([
                ...
                int(exit_code),
                bool(timed_out),
                (float(ee_to_target_dist_m) if ee_to_target_dist_m is not None else None),
                str(trial_dir),
            ])

    times = np.asarray(per_trial_times, dtype=np.float64) if per_trial_times else np.zeros(0)
    dists = np.asarray(per_trial_dists, dtype=np.float64) if per_trial_dists else np.zeros(0)
    agg = {
        "epoch": epoch,
        ...
        "num_trials_with_dist": int(dists.size),
        "mean_ee_to_target_dist_m": float(dists.mean()) if dists.size else None,
        "median_ee_to_target_dist_m": float(np.median(dists)) if dists.size else None,
        "min_ee_to_target_dist_m": float(dists.min()) if dists.size else None,
        "max_ee_to_target_dist_m": float(dists.max()) if dists.size else None,
    ...
    with csv_path.open("a", encoding="utf-8") as f:
        if new_file:
            f.write("epoch,num_trials,num_success,success_rate,mean_time_sec,"
                    ...
                    "num_trials_with_dist,mean_ee_to_target_dist_m\n")
        mean_dist_csv = (
            ""
            if agg["mean_ee_to_target_dist_m"] is None
            else f"{agg['mean_ee_to_target_dist_m']:.6f}"
        )
        f.write(
    ...
    if _wandb_active():
        payload: dict = {
            ...
            "integration/num_trials_with_dist": agg["num_trials_with_dist"],
        }
        ...
        try:
            trials_table = wandb.Table(
                columns=["epoch", "trial", "success", "elapsed_sec",
                         "exit_code", "timed_out", "ee_to_target_dist_m", "trial_dir"],
...
def train():
    ...
    if wandb_enabled and _WANDB_AVAILABLE:
        wandb_config = {
            ...
            "model": "StudentModelWonorm(ResNet18 pretrained -> MLP 512 -> z_dim)",
